...
class PNG:
    ...
    def one_IDAT_before_IEND(self,new_IDAT_chunk,secret_data):
        ...
        """
            RSA_len - length of RSA key in bits (must be mod 8 -> 8,16,24...).
            =========================================
            0. Zapamiętujemy ile bajtów miała zdekompresowana suma chankow IDAT -> len(self.IDAT_data) 
            1. Zdekompresowana suma chunkow IDAT (surowy obraz) -> ECB -> Kryptogram większy bajtowo
            2. Bierzemy tyle bajtów z krpytogramu ile wynosiła zapamiętana długosc (len(self.IDAT_data))
            3. Kompresujemy te bajty i zapisujemy do pojedynczego chunka IDAT (Kryptogram[0:zapamietany_rozmiar])
            4. Pozostała częsc (Kryptogram[zapamietany_rozmiar:do_konca]) leci w sekretna wiadomosc (za IEND i w CRC pustych chunkach IDAT)
        """    
    def write_encrypted_image_ECB(self,RSA_len):
        ...
        if self.rsa == None:
            self.rsa = RSA(RSA_len)  #self testowo na podgląd, ale no... tworzony tu klucz o zadanej dlugosci
            self.rsa.generate_keys() #wlasciwie to tu tworzony
        data_to_cipher = self.raw_image.ravel() # piksele w obrazie -> 1D array
        data_to_cipher = array_int_to_array_byte(data_to_cipher) #1D array int -> 1D array byte
        ciphered_data = self.rsa.crypto_ECB(data_to_cipher) #szyfrowanie 1D array byte -> 1D array wiekszy bajtowo od oryginału
        #trzeba ten wiekszy bajtowo szyfr rozdzielic na chunk IDAT i na sekretną wiadomosc, zeby byla mozliwosc odczytu pliku przez inne programy
        new_IDAT_chunk, additional_image_data = self.split_ciphered_data(ciphered_data) 
        new_IDAT_chunk = self.insert_filter_types(new_IDAT_chunk, self.width * self.bytesPerPixel) #dodanie filtru 0 na poczatek wiersza
        new_IDAT_chunk = zlib.compress(new_IDAT_chunk) 
        additional_image_data = zlib.compress(additional_image_data)
        self.one_IDAT_before_IEND(self.create_one_IDAT(new_IDAT_chunk), self.rsa.get_key_data()+additional_image_data) #utworzenie chunkow

        
        """
            1. Wczytanie informacji z sekretnej wiadomosci 
            2. Dekompresja dodatkowych bajtów obrazu z sekretnej wiadomosci
            3. Sklejenie IDAT bez filtrów (bez pierwszego bajtu w wierszu) z dodatkowymi bajtami
            4. Deszyfrowanie sklejonych bajtów
            5. Dodanie filtra 0 na początek każdego wiersza
        """
    def read_encrypted_image_ECB(self):
        secret_message = self.read_secret_message() #1
        ...
        additional_IDAT_data = secret_message[n_block_end:] #END 1
        additional_IDAT_data = zlib.decompress(additional_IDAT_data) #2
    ...
    def get_data_without_filter_byte(self):
        ...
        for i in range(self.height):
            start = i * row_len + (i + 1) #start on second byte in row
            data_without_filter +=self.IDAT_data[start:start + row_len]
        #print(f"{len(self.IDAT_data)} -> {len(data_without_filter)}")
    ...
    def split_ciphered_data(self,ciphered_data):
        ...
        IDAT_data = b''
        secret_data = b''
        # ciphered_data -> IDAT_data + secret_data
        block_start = 0
        while True:            
            block_end = block_start + self.rsa.block_length
            
            if block_end + 1 == ciphered_data_len: #LAST_BLOCK_EXCEPTION
                ...
                break            
            IDAT_data += ciphered_data[block_start:block_end] #transfer every byte but last one to IDAT_data
            block_start = block_end + 1                         #get new block_start
            secret_data += ciphered_data[block_end].to_bytes(1, byteorder = 'big') #last byte to secret_data                 
        return IDAT_data,secret_data

    def split_ciphered_data_CTR(self,ciphered_data):
        ciphered_data_len = len(ciphered_data)
        IDAT_data = b''
        secret_data = b''
        # ciphered_data -> IDAT_data + secret_data
    ...
    """
        IDAT data + secret_message_additional_IDAT_data
    """
    ...
    def insert_filter_types(self,content,row_length):
        ...
        zero = 0
        for i in range(self.height):
           # print(f"Dodaje {self.IDAT_data[i*(row_length+1)].to_bytes(1,byteorder='big')}")
            #return_data += self.IDAT_data[i*(row_length+1):i*(row_length+1)+1]
    ...
    def write_encrypted_image_CTR(self,RSA_len):
        def array_int_to_array_byte(data):
            i = 0
            byte_data = b''
            while i < len(data):
        ...
        data_to_cipher = self.raw_image.ravel() # piksele w obrazie -> 1D array
        data_to_cipher = array_int_to_array_byte(data_to_cipher) #1D array int -> 1D array byte
        ciphered_data = self.rsa.crypto_CTR(data_to_cipher) #szyfrowanie 1D array byte -> 1D array wiekszy bajtowo od oryginału
        #trzeba ten wiekszy bajtowo szyfr rozdzielic na chunk IDAT i na sekretną wiadomosc, zeby byla mozliwosc odczytu pliku przez inne programy
        new_IDAT_chunk, additional_image_data = self.split_ciphered_data_CTR(ciphered_data) 
        new_IDAT_chunk = self.insert_filter_types(new_IDAT_chunk, self.width * self.bytesPerPixel) #dodanie filtru 0 na poczatek wiersza
        new_IDAT_chunk = zlib.compress(new_IDAT_chunk) 
        additional_image_data = zlib.compress(additional_image_data)
        self.one_IDAT_before_IEND(self.create_one_IDAT(new_IDAT_chunk), self.rsa.get_key_data_public()+additional_image_data) #utworzenie chunkow

    def read_encrypted_image_CTR(self):
        secret_message = self.read_secret_message() #1
        d_len = int.from_bytes(secret_message[0:4], byteorder = 'big')
        n_len = int.from_bytes(secret_message[4:8], byteorder = 'big')
        last_block_len = int.from_bytes(secret_message[8:12], byteorder = 'big')
        ...
        self.rsa = RSA(n_len * 8, self.rsa.nonce)
        self.rsa.load_public_key(d,n)
        self.rsa.set_last_block_length(last_block_len)
     
        data_to_decrypt = self.get_data_without_filter_byte()
        data_to_decrypt = self.merge_bytes_data_CTR(data_to_decrypt,additional_IDAT_data)
        self.IDAT_data = self.rsa.decrypto_CTR(data_to_decrypt)
        self.IDAT_data = self.insert_filter_types(self.IDAT_data, self.width * self.bytesPerPixel)
        self.one_IDAT_before_IEND(self.create_one_IDAT(zlib.compress(self.IDAT_data)), b'')
        self.process_IDAT_image("ignore_decompression")
    ...
    def merge_bytes_data_CTR(self,IDAT_data,secret_data):
        block_no = 0
        block_start = 0
        block_end = 0
        merged_data = b''
        while block_start < len(IDAT_data):
            block_end = block_start + self.rsa.block_length + 1
            if block_end >= len(IDAT_data):
                merged_data += IDAT_data[block_start:]
                merged_data += secret_data[block_no:]
                break
